...
function GridColumn({ title, description, buttonUrl, buttonText, textColor, buttonTextColor, buttonBackgroundColor }) {
    return (
        <div className="flex flex-col items-center p-6" style={{ color: textColor }}>
            {title && <h1 className="text-xl font-bold m-3">{title}</h1>}
            {description && <p className="text-md m-3">{description}</p>}
            {buttonUrl && buttonText && (
                <Link href={buttonUrl}>
                    <div
                        className="flex items-center justify-center w-full px-4 py-2 text-md font-semibold rounded-full m-2"
                        style={{
