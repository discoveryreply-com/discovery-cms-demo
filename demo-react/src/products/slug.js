    ...
    return (
      ...
      <div className="mt-5 lg:flex lg:ml-20 mb-10 w-[80rem]">
        <div className="w-2/4 mt-[4rem]">
          <img src={productDetails.preview.url} alt="" className="w-full h-auto" />
        </div>
        <div className=" ml-1 mr-1 md:ml-5 lg:ml-16 w-2/4 mt-[4rem]">
          <div className="mt-5 mb-2 text-xs italic font-light text-gray-400">
            <div className="inline mr-1">Product</div>
            <div className=" inline mr-1">-</div>
            <div className=" inline mr-1">Collection</div>
            <div className=" inline mr-1">-</div>
            <div className=" inline mr-1">{productDetails.title}</div>
          </div>

          <div className="w-full text-3xl font-medium text-gray-800">{productDetails.title}</div>

          <div className="w-full font-light text-gray-400 text-sm mb-2">
            {productDetails.categories &&
              Object.keys(productDetails.categories).map(function (key) {
                return (
                  <span key={key} className="mr-2">
                    {productDetails.categories[key]}
                  </span>
                );
              })}
          </div>
          {productDetails?.description && (
            <div className="mt-5 text-md font-sans text-gray-500 md:max-w-md">
              {/* needed because the property is HTML */}
              <div className="font-sans" dangerouslySetInnerHTML={{ __html: productDetails.description }} />
            </div>
          )}
